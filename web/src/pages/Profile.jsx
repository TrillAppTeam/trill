...
function Profile() {
    ...
    const { isLoading, data: userData} = useQuery([`users${paramString}`], {onSuccess: (data) => {setIsFollowing(data.requestor_follows);}});
    const { data: following, refetch: refetchFollowing } = useQuery([`follows?type=getFollowing&username=${userData?.username}`], {enabled: !!userData});
    const { data: followers, refetch: refetchFollowers } = useQuery([`follows?type=getFollowers&username=${userData?.username}`], {enabled: !!userData});
    const { data: reviewsNew } = useQuery([`reviews?sort=newest&username=${userData?.username}`], {enabled: !!userData});
    const { data: reviewsPopular } = useQuery([`reviews?sort=popular&username=${userData?.username}`], {enabled: !!userData});
    const { isLoading: favoriteAlbumsLoading, data: favoriteAlbums } = useQuery([`favoritealbums?username=${userData?.username}`], {enabled: !!userData});
    
    const follow = useMutation(() => { 
        return axios.post(`https://api.trytrill.com/main/follows?username=${userData?.username}`, {}, 
            { headers: {'Authorization': `Bearer ${sessionStorage.getItem('access_token')}`}})
    }, {onSuccess: () => {setIsFollowing(true); refetchFollowing(); refetchFollowers();}});

    const unfollow = useMutation(() => { 
        return axios.delete(`https://api.trytrill.com/main/follows?username=${userData?.username}`, 
            { headers: {'Authorization': `Bearer ${sessionStorage.getItem('access_token')}`}})
        ...
        {isLoading ? <Loading/> : <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-row flex-wrap py-10 justify-between mx-10">
                <div className="flex flex-row m-5">
                    <Avatar user={{...userData, size: "24", linkDisabled: true}} />

                    <div className="pl-10">
                        <div className="flex gap-2">
                            <h1 className="font-bold text-white text-3xl">{userData?.nickname}</h1>
                ...
                <div className="pt-5">
                    <UserStats albums={userData?.review_count || 0} followers={followers?.length || 0} following={following?.length || 0}/>
                </div>
            </div>
           ...
           {/* Popular Reviews: Two most popular reviews by likes, by the user */}
            ...
            <div className="flex flex-col justify-center items-left max-w-5xl pb-10">
                {following?.length > 0 ? (
                    ...
                    {following?.map(user => {return <Avatar key={user.username} user={{profile_picture: user.profile_picture, username: user.username, size: '11'}}/>})}
                    </div>
                ) : (
           ...
           {/* Followers: Avatars of people that follow the user */}
            ...
            <div className="flex flex-col justify-center items-left max-w-5xl pb-10">
                {followers?.length > 0 ? (
                    <div className="flex gap-2 flex-wrap">
                    {followers?.map(user => {return <Avatar key={user.username} user={{profile_picture: user.profile_picture, username: user.username, size: '11'}}/>})}
                    </div>
                ) : (
